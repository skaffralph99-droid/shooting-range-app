...
const categoryColors = {
  ammo: 'bg-orange-900/40 text-orange-300 border-orange-700',
    ...
    <div>
      ...
      {showForm && (
        <div className="bg-gray-900 border border-orange-700 rounded-xl p-5 mb-6">
          <h2 className="text-white font-semibold mb-4">New Item</h2>
      ...
      <div className="bg-gray-900 border border-gray-800 rounded-xl overflow-hidden">
        <table className="w-full text-sm">
          ...
          <tbody className="divide-y divide-gray-800">
            {items.map(item => (
              <tr key={item.id} className={`hover:bg-gray-800/50 ${item.quantity <= item.min_stock ? 'bg-red-900/10' : ''}`}>
                ...
                <td className="px-4 py-3 text-gray-400">{item.caliber ?? '—'}</td>
                <td className="px-4 py-3 text-white font-bold">{item.quantity}</td>
                <td className="px-4 py-3 text-gray-400">{item.min_stock}</td>
                <td className="px-4 py-3 text-gray-300">${item.unit_cost}</td>
                <td className="px-4 py-3">
                  <div className="flex items-center gap-2">
                    <button onClick={() => updateQty(item.id, Math.max(0, item.quantity - 1))}
                      className="w-6 h-6 bg-gray-700 hover:bg-gray-600 text-white rounded text-xs flex items-center justify-center">−</button>
