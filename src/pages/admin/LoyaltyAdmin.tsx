...
export default function LoyaltyAdmin() {
  ...
  return (
    <div>
      <h1 className="text-2xl font-bold text-caliber-steel mb-6">Loyalty Management</h1>
      ...
      <div className="grid grid-cols-4 gap-3 mb-6">
        {[['🥉','Bronze',0,'#cd7f32'],['🥈','Silver',300,'#94a3b8'],['🥇','Gold',700,'#fbbf24'],['💎','Platinum',1500,'#e879f9']].map(([emoji, name, min, color]) => {
          ...
          return (
            <div key={name as string} className="bg-caliber-surface border border-caliber-border rounded-xl p-4 text-center">
              <div className="text-2xl mb-1">{emoji}</div>
              <p className="text-caliber-steel font-bold">{name}</p>
              <p className="text-3xl font-bold mt-1" style={{ color: color as string }}>{count}</p>
              <p className="text-caliber-muted text-xs">members</p>
            </div>
          )
        })}
      </div>
      ...
      {awarding && (
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4">
          <div className="bg-caliber-surface border border-caliber-gold/60 rounded-xl p-6 w-full max-w-sm">
            <h3 className="text-caliber-steel font-bold mb-4">Award Points</h3>
            <p className="text-caliber-dim text-sm mb-4">Member: {members.find(m => m.user_id === awarding.userId)?.full_name}</p>
            <input type="number" placeholder="Points to award" value={awarding.pts}
              onChange={e => setAwarding({...awarding, pts: e.target.value})}
              className="w-full bg-caliber-elevated border border-caliber-muted rounded-lg px-3 py-2 text-caliber-steel text-sm mb-3 focus:outline-none focus:border-caliber-gold" />
            <input placeholder="Reason" value={awarding.reason}
              onChange={e => setAwarding({...awarding, reason: e.target.value})}
              className="w-full bg-caliber-elevated border border-caliber-muted rounded-lg px-3 py-2 text-caliber-steel text-sm mb-4 focus:outline-none focus:border-caliber-gold" />
            <div className="flex gap-2">
              <button onClick={awardPoints} className="flex-1 py-2 bg-caliber-gold text-caliber-dark rounded-lg text-sm font-semibold">Award</button>
              <button onClick={() => setAwarding(null)} className="px-4 py-2 bg-caliber-muted text-caliber-steel rounded-lg text-sm">Cancel</button>
            </div>
          </div>
        </div>
      )}
      ...
      <div className="bg-caliber-surface border border-caliber-border rounded-xl overflow-hidden">
        <table className="w-full text-sm">
          ...
          <tbody className="divide-y divide-caliber-border">
            {members.map((m, i) => (
              <tr key={m.user_id} className="hover:bg-caliber-elevated/50">
                <td className="px-4 py-3">
                  <div className="flex items-center gap-2">
                    {i < 3 && <span className="text-sm">{['🥇','🥈','🥉'][i]}</span>}
                    <span className="text-caliber-steel font-medium">{m.full_name}</span>
                  </div>
                </td>
                <td className="px-4 py-3">
                  <span className="font-semibold" style={{ color: m.tier_color }}>{m.badge_emoji} {m.tier_name}</span>
                </td>
                <td className="px-4 py-3 text-caliber-gold font-bold">{m.total_points.toLocaleString()}</td>
                ...
                <td className="px-4 py-3">
                  <button onClick={() => setAwarding({ userId: m.user_id, pts: '', reason: '' })}
                    className="text-xs text-caliber-gold hover:text-caliber-gold-light transition-colors">+ Award Points</button>
                </td>
              </tr>
            ))}
          </tbody>
