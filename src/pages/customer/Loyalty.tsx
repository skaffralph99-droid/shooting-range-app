...
export default function Loyalty() {
  ...
  if (loading) return <div className="text-caliber-dim">Loading loyalty info...</div>
  ...
  return (
    <div className="max-w-3xl">
      <h1 className="text-2xl font-bold text-caliber-steel mb-6">💎 Loyalty Program</h1>

      {/* Tier Card */}
      <div className={`${currentTier.bg} border ${currentTier.border} rounded-2xl p-6 mb-6`}>
        <div className="flex items-center justify-between mb-4">
          <div>
            <p className="text-sm text-caliber-dim mb-1">Your Current Tier</p>
            <div className="flex items-center gap-2">
              <span className="text-4xl">{currentTier.emoji}</span>
              <span className="text-3xl font-bold text-caliber-steel">{currentTier.name}</span>
            </div>
          </div>
          <div className="text-right">
            <p className="text-sm text-caliber-dim">Total Points</p>
        ...
        {nextTier ? (
          <>
            <div className="flex justify-between text-xs text-caliber-dim mb-2">
              <span>{loyalty?.total_points ?? 0} pts</span>
              <span>{nextTier.min} pts → {nextTier.emoji} {nextTier.name}</span>
            </div>
            <div className="h-2.5 bg-caliber-elevated rounded-full overflow-hidden">
              <div className="h-full rounded-full transition-all duration-500"
                style={{ width: `${Math.min(100, progress)}%`, backgroundColor: currentTier.color }} />
            </div>
            <p className="text-sm text-caliber-dim mt-2">
              <span className="font-semibold text-caliber-steel">{loyalty?.points_to_next}</span> more points to reach {nextTier.emoji} {nextTier.name}
            </p>
          </>
        ) : (
      ...
      <div className="bg-caliber-gold/10 border border-caliber-gold/60 rounded-xl p-4 mb-6 flex items-center gap-4">
        <span className="text-3xl">🛍️</span>
        <div>
          <p className="text-caliber-steel font-bold text-lg">{currentTier.discount}% off all shop items</p>
          <p className="text-caliber-dim text-sm">Your {currentTier.name} tier discount is automatically applied at checkout</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-4 gap-3 mb-8">
        {TIERS.map(tier => (
          <div key={tier.name}
            className={`rounded-xl p-4 text-center border ${tier.name === currentTier.name ? `${tier.bg} ${tier.border}` : 'bg-caliber-surface border-caliber-border opacity-60'}`}>
            <div className="text-2xl mb-1">{tier.emoji}</div>
            <p className="font-bold text-caliber-steel text-sm">{tier.name}</p>
            <p className="text-xs text-caliber-dim mt-1">{tier.min.toLocaleString()}+ pts</p>
      ...
      {/* How to Earn */}
      <div className="bg-caliber-surface border border-caliber-border rounded-xl p-5 mb-6">
        <h2 className="text-caliber-steel font-bold mb-4">How to Earn Points</h2>
        <div className="space-y-3">
          {[
            ...
            <div key={item.label} className="flex items-center justify-between py-2 border-b border-caliber-border last:border-0">
              <div className="flex items-center gap-3">
                <span className="text-xl">{item.icon}</span>
                <span className="text-caliber-steel/80 text-sm">{item.label}</span>
              </div>
              <span className="text-caliber-gold font-semibold text-sm">{item.pts}</span>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="bg-caliber-surface border border-caliber-border rounded-xl overflow-hidden">
        <div className="px-5 py-4 border-b border-caliber-border">
          <h2 className="text-caliber-steel font-bold">Points History</h2>
        </div>
        {history.length === 0 ? (
          <p className="text-center text-caliber-muted py-8">No points earned yet. Book a lane or log a score!</p>
        ) : (
          <div className="divide-y divide-caliber-border">
            {history.map(evt => (
              <div key={evt.id} className="px-5 py-3 flex items-center justify-between">
                <div>
                  <p className="text-caliber-steel/80 text-sm">{evt.reason}</p>
                  <p className="text-caliber-muted text-xs mt-0.5">{format(new Date(evt.created_at), 'MMM dd, yyyy · HH:mm')}</p>
                </div>
